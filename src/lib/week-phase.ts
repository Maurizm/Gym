...
export function getPhaseInfo(): PhaseInfo {
  let settings = storage.get<{ startDate?: string; unit?: string; theme?: string }>('gymapp:settings', {});
  ...
  const start = new Date(settings.startDate as string);
